...
def save_gradient_images(gradient, file_name):
    ...
    """
    if not os.path.exists('generated2'):
        os.makedirs('generated2')
    # Normalize
    gradient = gradient - gradient.min()
    gradient /= gradient.max()
    # Save image
    path_to_file = os.path.join('generated2', file_name + '.png')
...
def preprocess_image(pil_im, resize_im=True):
    ...
    #im_as_arr = convert_to_grayscale(im_as_arr)
...
def recreate_image(im_as_var):
    ...
    for c in range(im_as_var.shape[0]):
